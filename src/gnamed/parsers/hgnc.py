...
from gnamed.constants import Species, Namespace
from gnamed.loader import \
    GeneRecord, AbstractLoader, DBRef, DuplicateEntityError
from gnamed.orm import GeneRef
    ...
    def _setup(self, stream: io.TextIOWrapper):
        ...
        try:
            ...
            if hgnc.id != entrez.id:
                hgnc.id = entrez.id
                logging.info(
                    'correcting wrong link of %s:31739 from '
                    '%s:388159 to %s:648809', Namespace.hgnc,
                    Namespace.entrez, Namespace.entrez)
                self.session.commit()
                # commit, because otherwise multiple
                # parsers would hang here waiting for
                # each others commit
